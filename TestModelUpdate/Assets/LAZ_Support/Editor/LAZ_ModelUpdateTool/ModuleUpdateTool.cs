...
public class ModuleUpdateTool : EditorWindow {
  ...
  void OnGUI () {
    ...
    if (GUILayout.Button ("更新至某版本(请查询log后填写5位数)", GUILayout.Height (20f))) {
      string tip = GitCheckOut (ModelUpdateConfig.LAZ_CheckTool, description);
      Debug.LogError (tip);
    }
    if (GUILayout.Button ("立即更新", GUILayout.Height (20f))) {
      string tips = GitUpdate(ModelUpdateConfig.LAZ_CheckTool);
      Debug.LogError(tips);
    }
  ...
  string GetGitLog (string logPath) {
    ...
    return Run(@"log --pretty=oneline " + path);
  }

  string GitCheckOut (string Path, string md5Code) {
    if (string.IsNullOrEmpty (Path)) {
      EditorUtility.DisplayDialog ("错误", "请联系制作工具相关人员并记录下操作过程", "确认", "取消");
      return string.Empty;
    }
    if (string.IsNullOrEmpty (md5Code)) {
      EditorUtility.DisplayDialog ("错误", "请输入正确的md5码(如无法确定请点解查看日志查询)", "确认", "取消");
      return string.Empty;
    }
    return Run(@"checkout " + md5Code);
  }

  string GitUpdate (string Path) {
    if (string.IsNullOrEmpty (Path)) {
      EditorUtility.DisplayDialog ("错误", "请联系制作工具相关人员并记录下操作过程", "确认", "取消");
      return string.Empty;
    }
